...
public class StartGameChains {
	...
	public static final ChainBuilder startGameChain = exec(
		http("start game")
			.post((Session session) -> "/game/" + session.getString("sessionId") + "/start")
			.headers(headers_0)
	)
		.exec(
			http("state after start")
