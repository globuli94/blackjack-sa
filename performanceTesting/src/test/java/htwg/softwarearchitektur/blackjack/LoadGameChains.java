...
public class LoadGameChains {
	...
	public static final ChainBuilder loadGameChain = exec(
		http("load game from persistence")
			.post((Session session) -> "/game/"
				+ session.getString("sessionId")
				+ "/load")
			.headers(headers_0)
	)
		.exec(
			http("get game state after load")
				.get((Session session) -> "/game/"
					+ session.getString("sessionId")
					+ "/state")
				.headers(headers_1)
		);
